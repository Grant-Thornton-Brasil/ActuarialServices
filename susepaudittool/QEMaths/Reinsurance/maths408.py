...
class maths_408():
    ...
    def run(self, linha):
        ...
        if MPATIPOCONT == "1" and MPAMODCONT in ["01","02"] and MPATIPOPERA == "1":
            if TPMORESSID in ["024","025","028"]:
                self.df["Cruzamento 1 - 408"][MRFMESANO] += float(MPAVALORMOVCOMIS)
            elif TPMORESSID in ["026","027"]:
                self.df["Cruzamento 1 - 408"][MRFMESANO] -= float(MPAVALORMOVCOMIS)
        # Cruzamento 3
        # MPATIPOCONT 1 ; MPAMODCONT 1+2 ; MPATIPOPERA 1 ; TPMORESSID
        ...
        # 12078
        if MPATIPOCONT in ["1","2"] and MPAMODCONT in ["1","2","3","4","5","6",99] and MPATIPOPERA == "2":
        ...
        if TPMORESSID in ["024", "025", "028", "029", "030", "032"]:
            self.df["Cruzamento 1 - 408"][MRFMESANO] += float(MPAVALORMOVCORRET)
